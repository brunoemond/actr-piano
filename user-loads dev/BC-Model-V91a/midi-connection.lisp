;; for this code to work you need to start fluidsynth on your computer as a server
;  fluidsynth -s 

(require 'usocket)

(defclass tcp-connection ()
  ((c-ip :accessor c-ip :initarg :ip :initform "127.0.0.1")
...
(defun make-midi-command (ichunk iconn)
  (let ((newtime (mp-time))*con*
    ...
    (setf tb (if (null (chunk-slot-value-fct ichunk 'bn)) 0
		 (chunk-slot-value-fct (chunk-slot-value-fct ichunk 'bn) 'cnum)))
    (setf t1 (if (null (chunk-slot-value-fct ichunk 'r1)) 0
		 (chunk-slot-value-fct (chunk-slot-value-fct ichunk 'r1) 'cnum)))
    (setf t2 (if (null (chunk-slot-value-fct ichunk 'r2)) 0
		 (chunk-slot-value-fct (chunk-slot-value-fct ichunk 'r2) 'cnum)))
    (setf t3 (if (null (chunk-slot-value-fct ichunk 'r3)) 0
		 (chunk-slot-value-fct (chunk-slot-value-fct ichunk 'r3) 'cnum)))
...
(defun make-midi-command-bn (ichunk iconn)
  (let ((newtime (mp-time))
    ...
    (setf (midi-pars-type-time *midi-pars*) newtime)
    (s-midi iconn tb 0)
    (setf tb (if (null ichunk) 0 (chunk-slot-value-fct ichunk 'cnum)))
    (s-midi iconn tb 1)
    (setf (midi-pars-type-tb *midi-pars*) tb)))
